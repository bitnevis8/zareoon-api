...
const { SERVICE_TYPES } = require("./model");
...
const create = async (req, res) => {
  try {
    ...
    if (!SERVICE_TYPES.includes(serviceType)) {
      return res.status(400).json({ success: false, message: "نوع خدمت نامعتبر است" });
    }
    ...
    res.status(201).json({ success: true, data: record, message: "درخواست ثبت شد" });
  } catch (error) {
...
const list = async (req, res) => {
  try {
    ...
    const items = await ServiceRequest.findAll({
      where,
...
const getById = async (req, res) => {
  try {
    ...
    console.error("Service request getById error:", error);
...
const updateStatus = async (req, res) => {
  try {
    ...
    const updated = await ServiceRequest.findByPk(item.id, {
      include: [{ model: User, as: "user", attributes: userAttrs }],
    });

    res.json({ success: true, data: updated, message: "به‌روزرسانی شد" });
  } catch (error) {
    console.error("Service request updateStatus error:", error);
